...
if shape==1 % Calculate the capacity of a rectangular section
    ...
    Ac=ones(1,number);
    for i=1:number
    Ac(i)=Ac(i)*b*stripe;  % Store the concrete element area
    end
    yc=(-1*h/2+stripe*0.5):stripe:h/2; % Store the concrete element coordinates
    As=ones(1,2);
    for i=1:2
    As(i)=As(i)*NumS*pi*d*d/4; % Store the steel element area 
    end
    ysi=h/2-c-dv-d/2;
    ys=[ysi,-1*ysi]; % Store the steel element coordinates
    ...
    strainC=0.0033;
     ...
     [M,N]=FEAmn(k); 
        ...
        if M>Mb
            Mb=M;
            Nb=N;      % Find the limiting axial force Nb
        end
    end
...
% Calculate the capacity of an annular section
    elseif shape==2    
        ...
        stripe=2*Rb/number;
        Ac=ones(1,number);
        for i=1:number    % Store the concrete element area
        ...
        yc=(-1*Rb/2+stripe*0.5):stripe:Rb/2; % Store the concrete element coordinates
        As=ones(1,((NumS-2)/2+2));
        for i=1:((NumS-2)/2+2)
            if  i==1||i==((NumS-2)/2+2)
                As(i)=As(i)*pi*d*d/4;
            else
                As(i)=As(i)*2*pi*d*d/4;clc% Store the steel element area 
             end
        end
        ...
        ys0=(Rb-c-dv-d/2)*sin(theta); % Store the steel element coordinates
        ys=[ys0,0,-1*ys0];
        % Case where the steel first reaches the ultimate tensile strain
        strainC=0:0.00001:0.0033; % Concrete strain at the compression edge
        MN2=zeros(length(strainC),2);  % Initialize the matrix to zero
 ...
 ylabel('Nu(kN)')
 hold off     
 all=[MN2;MN34;MN1];          % Combine all Nu and Mu calculation results
 AfterSort=sortrows(all,2);   % Sort Nb from small to large
  for i=1:length(AfterSort)   % Find the index corresponding to Nb-Mb
      if AfterSort(i,2)==Nb
          c=i;
      end
  end   
  nm1=AfterSort(1:c,:);                   % Split the two arrays at Nb
  nm2=AfterSort(c:length(AfterSort),:);
  fid=fopen('result.txt','w');
  fprintf(fid,'%9.4f %9.4f\n',AfterSort');% Write all results to result.txt
  fclose(fid);
  fid=fopen('resultA.txt','w');
  fprintf(fid,'%9.4f %9.4f\n',nm1');      % Write the results with N < Nb to resultA.txt
  fclose(fid);
  fid=fopen('resultB.txt','w');
  fprintf(fid,'%9.4f %9.4f\n',nm2');      % Write the results with N > Nb to resultB.txt
  fclose(fid);
